...
public class BatchSqlExecutor extends BaseSqlExecutor {
        ...
        try (StatementWrapper statementWrapper = new StatementWrapper(selectGeneratedKeys ? connection.prepareStatement(executableSql.getSqlList()[0], Statement.RETURN_GENERATED_KEYS)
            ...
            if (!sqlParametersList.isEmpty()) {
                for (SqlParameter[] sqlParameters : sqlParametersList) {
                    ...
                    //添加批处理
                    statementWrapper.statement.addBatch();
                }
            }
            //提交到数据库
            int[] updateCounts = statementWrapper.statement.executeBatch();
            //若是INSERT或UPDATE或DELETE，则返回更新的影响行数，否则返回查询结果集
            if (isDataModifyingSqlType) {
                //判断是否需要获取生成的主键值，如果需要则交给消费者处理
                if (selectGeneratedKeys)
                    handleGeneratedKeysOfInsert(updateCounts, consumer, statementWrapper.statement);
                statementWrapper.setResult(updateCounts);
                return statementWrapper;
            } else {
                statementWrapper.setResult(handleResultSetOfQuery(updateCounts, statementWrapper.statement, resultSetConvertor, typeMapping, executorContext.instanceMethodInvoker, executorContext.isAutoConvert()));
                return statementWrapper;
            }
    ...
    private Object handleResultSetOfQuery(int[] updateCounts, PreparedStatement preparedStatement, ResultSetConvertor resultSetConvertor, ObjectMap typeMapping, MethodInvoker instanceMethodInvoker, boolean autoConvert) throws SQLException, ConvertException {
        //收集产生的结果集

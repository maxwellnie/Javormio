...
public class QueryRowSqlFactory implements RowSqlFactory {
    ...
    public RowSql getRowSql(MetaData metaData) throws ExecutorException {
        ...
        QueryRowSql rowSql = instantiationRowSql(sqlType, params, columnSql, typeConvertors, fromSql, otherSql.toString());
        if (sqlDecorator != null && sqlDecorator.getLimitFragment() != null) {
            ...
            rowSql.setPaging(true);
            rowSql.setStart(sqlDecorator.getLimitFragment().getStart());
            rowSql.setOffset(sqlDecorator.getLimitFragment().getOffset());
        }
            ...
            if (sqlDecorator.getWhereFragment() != null) {
                ...
                for (int i = 0; i < sqlDecorator.getWhereFragment().getInlineSql().size(); i++) {
                    ...
                    for (Object obj : sqlDecorator.getWhereFragment().getInlineSql().get(i).getParams())
                        typeConvertors.add(TypeConvertorManager.getConvertor(obj.getClass()));
     ...
     * @return
     */
    private QueryRowSql instantiationRowSql(SqlType sqlType, List<List<Object>> params, StringBuilder columnSql, List<TypeConvertor<?>> typeConvertors, StringBuilder fromSql, String otherSql) {
        QueryRowSql rowSql = new QueryRowSql();

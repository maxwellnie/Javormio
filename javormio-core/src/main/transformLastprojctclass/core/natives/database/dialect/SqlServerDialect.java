...
public class SqlServerDialect extends Dialect {
    ...
    public QueryRowSql afterBuild(QueryRowSql rowSql) {
        ...
        String originalSql = rowSql.getNativeSql();
        String orderByPart = getOrderByPart(originalSql);
        String distinctStr = "";
        ...
        if (StringUtils.isNullOrEmpty(orderByPart)) {
            orderByPart = "ORDER BY CURRENT_TIMESTAMP";
        }
        long startIndex = rowSql.getStart() + 1;
        long endIndex = rowSql.getStart() + rowSql.getOffset();
        String sql = "WITH selectTemp AS (SELECT " + distinctStr + "TOP 100 PERCENT " +
                " ROW_NUMBER() OVER (" + orderByPart + ") as __row_number__, " + pagingBuilder +
                ") SELECT * FROM selectTemp WHERE __row_number__ BETWEEN " +
                startIndex + " AND " + endIndex + " ORDER BY __row_number__";

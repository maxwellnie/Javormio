...
public interface MethodHandler extends Comparable<MethodHandler> {
    /**
     * 序号越高，被代理的优先级越高，意味着这个方法处理器最先执行，处于调用链的最顶层。最高不建议超过9999L，因为框架为spring支持事务提供Session应该是最优先的。
     */
    ...
    /**
     * 序号越高，被代理的优先级越高。最高不建议超过9999L，因为框架为spring支持事务提供Session应该是最优先的。
     */
    long getIndex();
    ...
    class TargetMethodSignature {
        ...
        public String key() {
            return MethodExecutorUtils.getMethodDeclaredName(name, args);
        }

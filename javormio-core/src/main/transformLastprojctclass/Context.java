...
import com.maxwellnie.velox.sql.core.natives.database.mapping.DefaultReturnReturnTypeParser;
import com.maxwellnie.velox.sql.core.natives.database.mapping.ReturnTypeParser;
        ...
        for (Method method : daoImplClazz.getMethods()) {
            if (method.isAnnotationPresent(SQLMethod.class)) {
                ...
                if (SQLMethod.value() != null) {
                    tableInfo.registerReturnTypeMapping(StringUtils.getMethodDeclaredName(method), returnTypeParser.parse(method.getReturnType(), tableInfo));
                }
            }
        }
    }
